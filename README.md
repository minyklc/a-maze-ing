...
The package exposes two classes: `Box` (a single maze cell) and `Maze` (the full generator). To use it in any project, you have to install the ***`.whl`***, import `Maze`, instantiate it with your parameters, call `Maze().generate()` method, then access the grid, path, and pattern via attributes. \
The package can be rebuilt from source (`src` folder) at any time using `pyproject.toml` (with *`build`* package) and `setup.py` (a fallback/backup way which doesn't need *`build`* package), both provided at the root of the repository. See the section "**Building the package from source**" below.
...
__Checking mazegen-*.whl install__
```
  (bash)

pip show mazegen 2>/dev/null || pip install mazegen-1.0.0-py3-none-any.whl
python3 -c "from mazegen import Maze; print('OK')"
```
